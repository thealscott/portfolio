<header>
    <h2 id="intro" title="You like to read between the lines, huh?">
        Nivea<br>
        Soft<br>
    </h2>

    <p id="intro_addendum">
        This small site was part of a campaign for the Nivea Soft range. Nicola de Main had done the illustrations for the pots, and her fashion advice was dispensed through magazine style mood boards on the site. I was the sole dev on the project. 
        <br><br>
        The campaign is long done, but the site still exists in a little nook of Nivea's servers. 
    </p>
</header>

<section>
  
    <div class="button_wrapper">
        <a href="/work" class="button back_button">
            Back
        </a>
    </div>

    <div class="button_wrapper">
        <a href="http://ext11.nivea.co.uk/soft/" target="_blank" class="button">
            Check it out
        </a>
    </div>
    
    <figure>
        <a href="/images/work/niveasoft_1.jpg" target="_blank">
            <img src="/images/work/niveasoft_1.jpg" />
        </a>
    </figure>
    
    <figure>
        <a href="/images/work/niveasoft_2.jpg" target="_blank">
            <img src="/images/work/niveasoft_2.jpg" />
        </a>
    </figure>
    
</section>

<section>
    <figure>
        <a href="/images/work/niveasoft_3.jpg" target="_blank">
            <img src="/images/work/niveasoft_3.jpg" />
        </a>
    </figure>
    
    
    <figure>
        <a href="/images/work/niveasoft_4.jpg" target="_blank">
            <img src="/images/work/niveasoft_4.jpg" />
        </a>
    </figure>
</section>